package com.hb01.annotation;

import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.hibernate.Transaction;
import org.hibernate.cfg.Configuration;

public class RunnerSave01 {

    public static void main(String[] args) {

        Student01 student01 = new Student01();
        student01.setId(1001);
        student01.setName("Fatih Arzu");
        student01.setGrade(90);

        Student01 student02 = new Student01();
        student02.setId(1002);
        student02.setName("Arzu");
        student02.setGrade(85);

        Student01 student03 = new Student01();
        student03.setId(1003);
        student03.setName("Fatih");
        student03.setGrade(73);

        Student01 student04 = new Student01();
        student04.setId(1004);
        student04.setName("Ali");
        student04.setGrade(75);

        Student01 student05 = new Student01();
        student05.setId(1005);
        student05.setName("Ali");
        student05.setGrade(75);

        Configuration con = new Configuration().
                configure("hibernate.cfg.xml").addAnnotatedClass(Student01.class);
//   Hibernate e configuration  dosyami ve Entity class imi bildirdim.

        SessionFactory  sf  =  con.buildSessionFactory();
        Session session  = sf.openSession();
        Transaction tx = session.beginTransaction();

        session.save(student01);
        session.save(student02);
        session.save(student03);
        session.save(student04);
        session.save(student05);

        tx.commit(); //  commit yazmassaniz DB bilgilerimiz gitmez

        session.close();
        sf.close();


    }
}
